...
const AQuiz = () => {
  ...
  const handleStartQuiz = async () => {
    ...
    console.log(
      `Random screenshot times (ms): ${screenshotTimes.map((time) => Math.round(time))}`
    );
    ...
    if (videoRef.current) {
      ...
      canvas.toBlob((blob) => {
        if (blob) {
          ...
          const file = new File([blob], `screenshot-${index || Date.now()}.jpeg`, {
            type: "image/jpeg",
          });
...
//   }
  ...
  const handleSubmit = () => {
    const participant = opportunity?.participants.find(p => p.email === userInfo.email)
    const attempts = {
      ...participant,
      answers,
      screenshots,
